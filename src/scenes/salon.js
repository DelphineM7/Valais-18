import { colorizeBackground, drawBoundaries, fetchMapData,Instruction, DestroyInstruction, ShowObject, DestroyShowObject, startInteractionPNJ, SetSprite, ShowKey, PaiettesE} from "../utils.js";
...
export default async function salon(k){
    ...
    k.onKeyPress("enter",()=>{
        ...
        if (onCollidePapier){  
            ...
            ShowKey (k, "c",  "c-down", 1150, 55, 289, 359, 1, 0.2,"InstructionF")
            if(!Meet_Guillot_ok.getinstanceGuillot()){
    ...
    k.onKeyPress("e", async() =>{
        if (onCollidePapier && !CarnetOpen && !ReadingPapers){
            ...
            ShowKey (k, "fleche_droite",  "fleche_droite-down", 990, 400, 289, 359, 1, 0.3,"InstructionCarnetDroite")
            ShowKey (k, "enter",  "enter-down", 970, 200, 289, 359, 1, 0.3,"InstructionExitCarnet")
    ...
    k.onKeyPress((key) => {if(["left","a"].includes(key)){
        ...
        k.onClick("Adrien_Felix",() =>     createProof(k,2, entities.carnet, Pottier_ok.getinstancePottier(), Meet_Pottier_ok.getinstancePottier(),k.vec2(305,445),285, 25,k.vec2(305,470),285, 50, "proof1_Pottier", "proof2_Pottier"))
        k.onClick("Alphonse_Beck",() =>    createProof(k,1, entities.carnet, Beck_ok.getinstanceBeck(), SeenJournal.getInstanceJournal(),k.vec2(390,250),Beck_proof1_width, 20,k.vec2(Beck_proof2_x,Beck_proof2_y),50 , 20, "proof1_Beck", "proof2_Beck"))
    ...
    entities.big_player.onCollide("papier_2", ()=>{
        ...
        ShowKey (k, "e","e-down", 950, 300, 225, 240, 1, 0.2,"InstructionE")
    })
    ...
    entities.big_player.onCollide("pnj-Pottier", () =>{
        ...
        ShowKey (k, "e","e-down", 570, 220, 225, 240, 1, 0.2,"InstructionEPottier")
    }); 
